...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/_vinext/image") {
      ...
      // If the image optimizer is not configured (local dev), serve the original asset directly.
      if (!env.IMAGES) {
        const assetPath = url.searchParams.get("url");
        if (assetPath) {
          // Use the ASSETS binding if available, otherwise fall back to a direct fetch.
          if (env.ASSETS) {
            return env.ASSETS.fetch(new Request(new URL(assetPath, request.url)));
          }
          return fetch(new URL(assetPath, request.url));
        }
      }
